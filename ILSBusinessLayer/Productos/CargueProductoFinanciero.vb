...
Public Class CargueProductoFinanciero
        ...
        Try
            ...
            If _estructuraTablaBase Is Nothing Then
                With dtDatos.Columns
                    ...
                    .Add(New DataColumn("cantidad", GetType(Long)))
        ...
        Try
            ...
            For Each registro As DataRow In dtDatos.Rows
                ...
                With registroFinal
                    ...
                    .Item("cantidad") = registro("cantidad").ToString.Trim
            ...
            For Each fila As ExcelRow In oExcel.Worksheets(0).Rows
                ...
                ElseIf index > 1 Then
                    ...
                    If String.IsNullOrEmpty(fila.Cells(1).Value) Then
                        AdicionarError(index, "Dato inválido", "La cantidad no puede estar vacía.")
                    Else
                        If Not IsNumeric(fila.Cells(1).Value) OrElse Not Integer.TryParse(fila.Cells(1).Value, numInte) Then _
                            AdicionarError(index, "Dato inválido", "La cantidad debe ser un dato numérico.")
                    ...
                    Else
                        If Not IsNumeric(fila.Cells(2).Value) OrElse Not Integer.TryParse(fila.Cells(2).Value, numInte) Then _
                        AdicionarError(index, "Dato inválido", "El centro debe ser numérico.")
                    End If

                    If String.IsNullOrEmpty(fila.Cells(3).Value) Then
                        AdicionarError(index, "Dato inválido", "El almacén no puede estar vacío.")
                    Else
                        If Not IsNumeric(fila.Cells(3).Value) OrElse Not Integer.TryParse(fila.Cells(3).Value, numInte) Then _
                        AdicionarError(index, "Dato inválido", "El almacén debe ser numérico.")
                    End If
                End If
                index += 1
            Next
        ...
        Try
            ...
            Using dbManager As New LMDataAccess
                With dbManager
                    ...
                    With .BulkCopy
                        .DestinationTableName = "TransitoriaInventarioFinanciero"
                    ...
                    .SqlParametros.Clear()
                    .SqlParametros.Add("@idUsuario", SqlDbType.Int).Value = idUsuario
                    _estructuraTablaErrores = .EjecutarDataTable("ValidarDatosInventarioFinanciero", CommandType.StoredProcedure)
    ...
    Function CargarInventario() As ResultadoProceso
        ...
        Try
            ...
            With dbManager
                ...
                End With
                .IniciarTransaccion()
                .EjecutarNonQuery("RegistrarInventarioFinanciero", CommandType.StoredProcedure)
