...
Public Class InventarioCargueProductoFinanciero
        ...
        Try
            ...
            If _estructuraTablaBase Is Nothing Then
                With dtDatos.Columns
                    ...
                    .Add(New DataColumn("consecutivo", GetType(String)))
        ...
        Try
            ...
            For Each registro As DataRow In dtDatos.Rows
                ...
                With registroFinal
                    ...
                    .Item("consecutivo") = registro("consecutivo").ToString.Trim
            ...
            For Each fila As ExcelRow In oExcel.Worksheets(0).Rows
                ...
                ElseIf index > 1 Then
                    ...
                    If String.IsNullOrEmpty(fila.Cells(1).Value) Then
                        AdicionarError(index, "Dato inválido", "El consecutivo no puede estar vacío.")
                    End If

                    If String.IsNullOrEmpty(fila.Cells(2).Value) Then
                        AdicionarError(index, "Dato inválido", "El centro no puede estar vacío.")
                    Else
                        If String.IsNullOrEmpty(fila.Cells(2).Value) Then _
                        AdicionarError(index, "Dato inválido", "El centro no puede estar vacío.")
                    End If
                    ...
                    Else
                        If String.IsNullOrEmpty(fila.Cells(3).Value) Then _
                        AdicionarError(index, "Dato inválido", "El centro no puede estar vacío.")
                    End If
                End If
                index += 1
        ...
        Try
            ...
            Using dbManager As New LMDataAccess
                With dbManager
                    ...
                    .SqlParametros.Clear()
                    .SqlParametros.Add("@idUsuario", SqlDbType.Int).Value = idUsuario
                    _estructuraTablaErrores = .EjecutarDataTable("ValidarDatosInventarioFinancieroSerializadoProducto", CommandType.StoredProcedure)
    ...
    Function CargarInventario() As ResultadoProceso
        ...
        Try
            ...
            With dbManager
                ...
                End With
                .IniciarTransaccion()
                .EjecutarNonQuery("RegistrarInventarioFinancieroSerializadoProducto", CommandType.StoredProcedure)
        ...
        Try
            ...
            With dbManager
                With .SqlParametros
                    ...
                    .Add("@resultado", SqlDbType.Int).Direction = ParameterDirection.ReturnValue
                End With
                .ejecutarReader("ObtenerProductosFinancierosTransitorios", CommandType.StoredProcedure)

                If .Reader IsNot Nothing Then
                    If .Reader.HasRows Then
                        Dim listSeriales As New List(Of String)
                        'Dim arraySeriales() As String
                        While .Reader.Read
                            listSeriales.Add(.Reader("serial").ToString)
                        End While
                        .Reader.Close()
                        'arraySeriales = listSeriales.ToArray
