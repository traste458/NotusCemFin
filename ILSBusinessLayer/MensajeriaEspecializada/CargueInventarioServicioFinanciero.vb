...
Public Class CargueInventarioServicioFinaciero
        ...
        Try
            ...
            If _estructuraTablaBase Is Nothing Then
                With dtDatos.Columns
                    .Add(New DataColumn("identificacionCliente", GetType(String)))
                    .Add(New DataColumn("codigoMaterial", GetType(String)))
                    .Add(New DataColumn("serial", GetType(String)))
                    .Add(New DataColumn("NumeroRadicado", GetType(String)))
                End With
    ...
    Private Sub AdicionarColumnas()
        Try
            ...
            For Each registro As DataRow In dtDatos.Rows
                ...
                With registroFinal
                    .Item("identificacionCliente") = registro("identificacionCliente").ToString.Trim
                    .Item("codigoMaterial") = registro("codigoMaterial").ToString.Trim
                    .Item("serial") = registro("serial").ToString.Trim
                    .Item("NumeroRadicado") = registro("NumeroRadicado").ToString.Trim
                End With
...
#Region "Métodos Públicos"
    Public Function ConsultarCliente() As DataTable
        ...
        Try
            With dbManager
                With .SqlParametros
                    .Clear()
                    .Add("@cedulaCliente", SqlDbType.VarChar).Value = cedulaCliente
                End With
                dtDatos = .EjecutarDataTable("ObtenerBodegasServiciosCliente", CommandType.StoredProcedure)
            End With
        Finally
    ...
    Public Function ConsultarMateriales() As DataTable
        Dim dtDatos As New DataTable
        If dbManager IsNot Nothing Then dbManager = New LMDataAccess
        Try
            With dbManager
                ...
                End With
                dtDatos = .EjecutarDataTable("ObtenerMaterialesServiciosCliente", CommandType.StoredProcedure)
            End With
        Finally
            If dbManager IsNot Nothing Then dbManager.Dispose()
        End Try
    ...
    Public Sub RegistrarSerialFinanciero()
        Try
            With dbManager
                ...
                .IniciarTransaccion()
                .EjecutarNonQuery("registrarSerialServicioFinanciero", CommandType.StoredProcedure)
                ...
                If Not IsDBNull(.SqlParametros("@resultado").Value) Then
                    ...
                    If resultado.Valor = 1 Then
                        ...
                        ObtenerInventarioProductoFinanciero()
                    Else
            ...
            For Each fila As ExcelRow In oExcel.Worksheets(0).Rows
                ...
                ElseIf index > 1 Then
                    ...
                    If String.IsNullOrEmpty(fila.Cells(1).Value) Then
                        AdicionarError(index, "Dato inválido", "El codigo del material no puede estar vacío.")
                    Else
                        dato = fila.Cells(1).Value
                        If dato.Length > 10 Then _
                            AdicionarError(index, "Dato inválido", "La longitud del codigo del material no es válida.")

                        If Not oExpReg.IsMatch(fila.Cells(1).Value) Then _
                                AdicionarError(index, "Dato inválido", "La cadena de carateres ingresados para el codigo del material no son válidas.")
                    End If

                    dato = fila.Cells(2).Value
                    If dato IsNot Nothing Then
                        If Not IsNumeric(fila.Cells(2).Value) OrElse Not Double.TryParse(fila.Cells(2).Value, numInte) Then
                    ...
                    End If
                    dato = fila.Cells(3).Value
                    If dato IsNot Nothing Then
                        If Not IsNumeric(fila.Cells(3).Value) OrElse Not Double.TryParse(fila.Cells(3).Value, numInte) Then
                            AdicionarError(index, "Dato inválido", "El numero de radicado tiene que ser numerico")
                        End If
                    Else
                        AdicionarError(index, "Dato inválido", "El campo radicado no puede estar vacio.")
                    End If
        ...
        Try
            ...
            Using dbManager As New LMDataAccess
                With dbManager
                    .SqlParametros.Clear()
                    .SqlParametros.Add("@idUsuario", SqlDbType.Int).Value = idUsuario
                    .EjecutarNonQuery("LiberarDatosTransitoriosInventario", CommandType.StoredProcedure)

                    .InicilizarBulkCopy(SqlClient.SqlBulkCopyOptions.UseInternalTransaction)
                    .TiempoEsperaComando = 0
                    With .BulkCopy
                        .DestinationTableName = "TransitoriaInventarioPersonalizado"
                        .ColumnMappings.Add("identificacionCliente", "identificacionCliente")
                        .ColumnMappings.Add("codigoMaterial", "codigoMaterial")
                        .ColumnMappings.Add("serial", "serial")
                        .ColumnMappings.Add("idUsuario", "idUsuario")
                        .ColumnMappings.Add("NumeroRadicado", "idServicioMensajeria")
                        .WriteToServer(EstructuraTablaBase)
                    End With

                    .SqlParametros.Clear()
                    .SqlParametros.Add("@idUsuario", SqlDbType.Int).Value = idUsuario
                    _estructuraTablaErrores = .EjecutarDataTable("ValidarDatosInventarioPersonalizado", CommandType.StoredProcedure)
    ...
    Function RegistrarInventario() As ResultadoProceso
        ...
        Try
            ...
            With dbManager
                ...
                End With
                .IniciarTransaccion()
                .ejecutarReader("RegistrarInventarioPersonalizadoMasivo", CommandType.StoredProcedure)
