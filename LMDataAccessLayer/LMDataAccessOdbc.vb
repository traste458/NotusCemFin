...
#Region "variables"
    ...
    ''' <summary>
    ''' command encargado de ejecutar las sentencias ODBC toda la clase
    ''' </summary>
    ''' <remarks></remarks>
    Private comando As OdbcCommand
    Private transaccion As OdbcTransaction
    'Private myBulkCopy As SqlBulkCopy 'Pendiente
    Private _infoConexionOdbc As InfoObjetoConexionOdbc
    Private _reader As OdbcDataReader
    Private pOdbc As New OdbcPermission(System.Security.Permissions.PermissionState.Unrestricted)
#End Region
...
#Region "Propiedades"
    ...
    Public ReadOnly Property EstadoTransaccional() As Boolean
        Get
            If transaccion Is Nothing OrElse transaccion.Connection Is Nothing OrElse _
    ...
    'Public ReadOnly Property BulkCopy() As SqlBulkCopy 'Pendiente
    '    Get
    '        Return myBulkCopy
    '    End Get
    'End Property

    Public ReadOnly Property TransaccionSQL() As OdbcTransaction
        Get
            Return transaccion
        End Get
    End Property

    Public ReadOnly Property ConexionSQL() As OdbcConnection
        Get
            Return conexion
        End Get
    End Property

    Public ReadOnly Property SqlParametros() As OdbcParameterCollection
        Get
    ...
    Property OdbcParametros(ByVal index As Integer) As OdbcParameter
        Get
            ...
            End If
        End Get
        Set(ByVal Value As OdbcParameter)
            If comando.Parameters.Count > 0 Then
    ...
    ''' <returns>SqlClient.SqlParameter</returns>
    ''' <remarks>se puede utilizar para acceder directamente los paremetros establecidos al command de la clase</remarks>
    Property OdbcParametros(ByVal Nombre As String) As OdbcParameter
        Get
            If comando.Parameters.Count > 0 Then
    ...
    Property TiempoEsperaComando() As Integer
        ...
        Set(ByVal value As Integer)
            ...
            'If myBulkCopy IsNot Nothing Then myBulkCopy.BulkCopyTimeout = value 'Pendiente
        End Set
    End Property
    ...
    ''' </summary>
    Public ReadOnly Property InformacionDeConexion() As InfoObjetoConexionOdbc
        Get
            Dim builder As New OdbcConnectionStringBuilder(conexion.ConnectionString)
            _infoConexionOdbc = New InfoObjetoConexionOdbc
            With _infoConexionOdbc
...
#Region "Contructores +/Destructor"
    ...
    Public Sub New()
        pOdbc.Assert()
        conexion = New OdbcConnection(ConfigurationManager.AppSettings("CadenaConexion"))
        comando = conexion.CreateCommand
        comando.Connection = conexion
    ...
    Public Sub Dispose() Implements System.IDisposable.Dispose
        ...
        'If myBulkCopy IsNot Nothing Then myBulkCopy.Close() 'Pendiente
        CcerrarConexion(True)
        If conexion IsNot Nothing Then conexion.Dispose()
        _infoConexionOdbc = Nothing
        GC.Collect()
        Dispose(True)
    End Sub
...
#Region "Consultas"
    ...
    Public Function ejecutarReader(ByVal sentencia As String, Optional ByVal tipoComando As CommandType = CommandType.Text) As OdbcDataReader
    ...
    ''' 
    Public Function GenerarArchivoExcel(ByVal sentencia As String, ByVal pNombreArchivo As String, Optional ByVal tipoComando As CommandType = CommandType.Text, _
        ...
        If Me._reader IsNot Nothing AndAlso Me._reader.HasRows Then
            ...
            If System.IO.File.Exists(pNombrePlantilla) Then
                Select Case System.IO.Path.GetExtension(pNombrePlantilla)
                    Case ".xls"
...
#End Region
    ...
    ''' </summary>
    Public Shared Function ObtenerInformacionDeConexion() As InfoObjetoConexionOdbc
        Dim infoConexion As New InfoObjetoConexionOdbc
        Dim builder As New OdbcConnectionStringBuilder(ConfigurationManager.AppSettings("CadenaConexionodbc"))
        With infoConexion
            .CadenaConexion = builder.ConnectionString
            '.NombreBaseDatos = builder.InitialCatalog 'Pendiente
            '.NombreServidor = builder.DataSource 'Pendiente
            '.TiempoEspera = builder.ConnectTimeout 'Pendiente
            '.NombreUsuario = builder.UserID 'Pendiente
            '.Password = builder.Password 'Pendiente
        End With
        builder.Clear()
        Return infoConexion
    End Function
...
Public Structure InfoObjetoConexionOdbc
...
Public Class InfoResultadoOdbc
