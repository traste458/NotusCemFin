...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                ...
                epNotificador.setTitle("Creación de Órdenes de Recolección")
                CargarListadoOrigen()
    ...
    Private Sub CargaInicial()
        Try
            ...
            dt = ILSBusinessLayer.Despachos.TipoDespacho.ListadoTipoArticulos()
            MetodosComunes.CargarDropDown(dt, CType(ddlTipoArticulo, ListControl))

            Me.ObtenerMateriales(0)
            pnlSeriales.Visible = False
            lnkVerTodos.Visible = False

            Session.Remove("dtReferencias")
            Session.Remove("dtAccesorios")
            Session.Remove("dtSeriales")
            Session.Remove("dtvalorMaterial")
...
#Region "MANEJO DE DESTINOS"
    ...
    Private Sub CargarListadoDeDestinatarios(ByVal filtroRapido As String, ByVal ddl As DropDownList, ByVal opcionInicial As String)
        Try
            ...
            End With
        Catch ex As Exception
            epNotificador.showError("Error al tratar de cargar datos. " & ex.Message)
        End Try
    End Sub
    ...
    Private Function ObtenerListaDestinatario(ByVal filtroRapido As String) As DataTable
        ...
        If dt IsNot Nothing Then
            ...
            dcAux.Expression = "ISNULL(idcliente2,' ')+' - '+cliente+' '+direccion+' '+ ciudad"
...
#Region "MANEJO DE REFERENCIAS"
    ...
    Protected Sub ddlMaterial_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ddlMaterial.SelectedIndexChanged
        If ddlMaterial.SelectedIndex > 0 Then
            ...
            lblValorMaterial.Visible = False
        End If
    End Sub
...
#Region "Metodos Referencias"
    ...
    Private Sub AgregarReferencia(ByVal dtReferencias As DataTable, Optional ByVal detalle As OrdenRecoleccionDetalle = Nothing)
        ...
        If ref.Length = 0 Then
            ...
            dtmateriales = ILSBusinessLayer.Productos.Producto.ValoresMateriales(ddlMaterial.SelectedValue)

            If origen = "manual" Then
                If dtmateriales.Rows.Count > 0 Then
                    If dtmateriales.Rows(0).Item("precio") = 0 Then
                        lblValorMaterial.Visible = True
                        Exit Sub
                    Else
                        lblValorMaterial.Visible = False
                    End If
            ...
            If detalle Is Nothing Then
                ...
                If origen = "manual" Then
                    dr("valorMaterial") = dtmateriales.Rows(0).Item("precio") * txtCantidad.Text.Trim
                Else
                    dr("valorMaterial") = 0
                End If
            ...
            End If
        Else
            epNotificador.showWarning("La referencia ya se encuentra agregada en esta recolección")
        End If

    End Sub

    Private Sub CalcularValorMaterial(ByVal dtReferencias As DataTable, ByVal dtErrores As DataTable)

        Dim dtmateriales As DataTable
        dtmateriales = ILSBusinessLayer.Productos.Producto.ValoresMateriales("")

        For i As Integer = 0 To dtReferencias.Rows.Count - 1
            Dim ref As DataRow() = dtmateriales.Select("material='" & dtReferencias.Rows(i).Item("material") & "'")
            If ref.Length > 0 Then
                If ref(0).Item("precio") = 0 Then
                    registrarErrorEnArchivo(dtErrores.Rows.Count + 1, "El material " & dtReferencias.Rows(i).Item("material") & " no tiene valor.", dtErrores)
                Else
                    dtReferencias.Rows(i).Item("valorMaterial") = dtReferencias.Rows(i).Item("cantidad") * ref(0).Item("precio")
                End If
            Else
                registrarErrorEnArchivo(dtErrores.Rows.Count + 1, "El material " & dtReferencias.Rows(i).Item("material") & " no existe en la base de datos de precios.", dtErrores)
            End If
        Next
    End Sub
...
#Region "MANEJO TRANSPORTADORAS"
    Private Sub ddlTransportadora_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ddlTransportadora.SelectedIndexChanged
        If ddlTransportadora.SelectedValue = 8 Then 'Setecsa
            pnlAccesorios.Enabled = False
        Else 'Tcc - Envia
            pnlAccesorios.Enabled = True
        End If
    End Sub
#End Region
...
#Region "Metodos seriales"
    ...
    Private Sub AgregarSeriales(ByVal dtSeriales As DataTable)
        ...
        Else
            epNotificador.showWarning("El serial " & txtSerial.Text & " ya se encuentra leido en esta recolección")
        End If
    End Sub
...
#Region "MANEJO DE ACCESORIOS"
    ...
    Protected Sub lnkAgregarAccesorio_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkAgregarAccesorio.Click
        ...
        txtCantidadAccesorio.Enabled = False
        txtAccesorio.Enabled = False
        ddlTipoArticulo.Enabled = False
        txtValorArticulo.Enabled = False
        lnkAgregarAccesorio.Visible = False
    End Sub

    Private Sub AgregarAccesorio(ByVal dtAccesorios As DataTable)
        lblAccesorioWarning.Text = ""
        If dtAccesorios.Select("articulo='" & txtAccesorio.Text.Trim & "'").Length = 0 Then
            Dim dr As DataRow = dtAccesorios.NewRow()
            dr("articulo") = txtAccesorio.Text.Trim
            dr("cantidadPedida") = txtCantidadAccesorio.Text
            dr("tipoArticulo") = ddlTipoArticulo.SelectedValue
            dr("tipoArticuloDescripcion") = ddlTipoArticulo.SelectedItem.Text.Trim
            dr("valorArticulo") = txtValorArticulo.Text.Trim
            dtAccesorios.Rows.Add(dr)
            Me.CargarGridAccesorios(dtAccesorios)
        Else
            lblAccesorioWarning.Text = "<ul><li>ya se agregó un articulo con la misma descripcion</li></ul>"
        End If
    ...
    Private Sub gvAccesorios_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvAccesorios.RowCommand
        Select Case e.CommandName
            Case "Eliminar"
                ...
                ddlTipoArticulo.SelectedValue = 0
                txtValorArticulo.Text = ""
                txtCantidadAccesorio.Enabled = True
                txtAccesorio.Enabled = True
                ddlTipoArticulo.Enabled = True
                txtValorArticulo.Enabled = True
                lnkAgregarAccesorio.Visible = True
        End Select
    End Sub
#End Region
...
#Region "Carga de datos por archivo plano"
    Private Sub lnkCargarSeriales_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lnkCargarSeriales.Click
        ...
        Try
            If FileUploadSeriales.PostedFile.FileName <> "" Then
                TargetPath = Server.MapPath("../archivos_planos/") & idUsuario & FileUploadSeriales.FileName
            ...
            End If
        Catch ex As Exception
            epNotificador.showError("Error al tratar de subir el archivo: " & TargetPath)
        End Try
    ...
    Public Sub CargarSeriales(ByVal ruta As String)
        ...
        Try
            dtSeriales = OrdenRecoleccionSerial.ObtenerEstructuraDeDatos
            dtSeriales.Columns.Add(New DataColumn("material", GetType(String)))
            dtSeriales.Columns.Add(New DataColumn("referencia", GetType(String)))
            dtReferencias = OrdenRecoleccionDetalle.ObtenerEstructuraDeDatos
            'carga los seriales del archivo plano a un datatable
            ...
            Do While Not strLine Is Nothing
                ...
                If arrSerial.Count = 3 Then
                    ...
                    Dim expresion As String = RegularExpressionValidatorSerial.ValidationExpression
                    Dim oRegex As New Regex(expresion)
                    If Not oRegex.IsMatch(arrSerial(0)) Then registrarErrorEnArchivo(lineaArchivo, "El Serial no tiene una longitud correcta", dtErrores, arrSerial(0))
                    fila("serial") = arrSerial(0)
                    fila("material") = arrSerial(1)
                    ' fila("referencia") = arrSerial(2)
                    Dim cajaVacia As Boolean
                    If arrSerial(2) = "1" Then
                        fila("cajaVacia") = 1
                    ElseIf arrSerial(2) = "0" Then
                        fila("cajaVacia") = 0
                    Else
                        registrarErrorEnArchivo(lineaArchivo, "la columna caja vacia debe tener valor uno o cero", dtErrores)
                    End If
                    fila("lineaArchivo") = lineaArchivo
                    fila("idUsuario") = idUsuario
                    dtSeriales.Rows.Add(fila)
                Else
                    registrarErrorEnArchivo(lineaArchivo, "El formato de las columnas del archivo no es el esperado", dtErrores)
                End If
                strLine = lector.ReadLine
            Loop
            lector.Close()
            If dtErrores.Rows.Count = 0 Then
                ...
                CalcularValorMaterial(dtReferencias, dtErrores)
                ...
                If dtErrores.Rows.Count = 0 Then
                    ...
                    Dim flagCorrectos As Boolean = ordSerial.ValidarSerialesRecoleccion(idUsuario)
                    If flagCorrectos Then
                        Me.CargarGridMateriales(dtReferencias)
                        lnkVerTodos.Visible = True
                    Else
                        dtSeriales.Rows.Clear()
                        dtReferencias.Rows.Clear()
                        epNotificador.showWarning("Los registros en el archivo no cumplen con algunas validaciónes")
                ...
                End If
            Else
                epNotificador.showWarning("El archivo no tiene el formato de columnas esperado")
                cargarGridErrores(dtErrores)
            End If
        Catch ex As Exception
            dtReferencias.Rows.Clear()
            dtSeriales.Rows.Clear()
            epNotificador.showError("Error al cargar archivo: " & ex.Message)
        Finally
            lector.Dispose()
        End Try
    End Sub

    Private Sub registrarErrorEnArchivo(ByVal lineaArchivo As Integer, ByVal mensajeError As String, ByVal dtErrores As DataTable, Optional ByVal serial As String = "")
        Dim dr As DataRow = dtErrores.NewRow
        dr("lineaArchivo") = lineaArchivo
        dr("mensajeError") = mensajeError
        If dtErrores.Columns.Contains("serial") Then dr("serial") = serial
...
#End Region
    ...
    Protected Sub lnkGuardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lnkGuardar.Click
        ...
        If Me.ContieneReferencias Or Me.contieneAccesorios Then
            ...
            If txtGuia.Text.Trim = "" And txtOrdenServicio.Text.Trim = "" Then
                epNotificador.showWarning("La recolección debe contener Número de Guia o Número de Orden de Servicio")
                Exit Sub
            End If

            'datos generales de la orden
            Dim orden As New OrdenRecoleccion
            ...
            orden.IdCreador = idUsuario
            orden.Observacion = txtObservacion.Text
            orden.Referencias.ListaReferencias = dtReferencias
            orden.Referencias.Seriales.ListaSeriales = dtSeriales
            orden.Accesorios.ListaAccesorios = dtAccesorios
            Long.TryParse("0", orden.ValorDeclarado)

            'Valida Configuracion
            Dim i As Integer
            If Not dtReferencias Is Nothing Then
                For i = 0 To dtReferencias.Rows.Count - 1
                    Dim dt As DataTable
                    dt = orden.validarConfiguracion(0, dtReferencias.Rows(i).Item("idproducto"), ddlOrigen.SelectedValue, ddlDestino.SelectedValue, ddlTransportadora.SelectedValue)
                    If dt.Rows.Count = 0 Then
                        epNotificador.showWarning("La recolección  no tiene configuracion de Matriz de Transporte")
                        Exit Sub
                    End If
                Next
            End If
            i = 0
            If Not dtAccesorios Is Nothing Then
                For i = 0 To dtAccesorios.Rows.Count - 1
                    Dim dt As DataTable
                    dt = orden.validarConfiguracion(dtAccesorios.Rows(i).Item("tipoArticulo"), 0, ddlOrigen.SelectedValue, ddlDestino.SelectedValue, ddlTransportadora.SelectedValue)
                    If dt.Rows.Count = 0 Then
                        epNotificador.showWarning("La recolección  no tiene configuracion de Matriz de Transporte")
                        Exit Sub
                    End If
            ...
            Try
                ...
                If flagCorrectos Then
                    orden.Crear()
                    Response.Redirect("BusquedaOrdenesRecoleccion.aspx?ok=" & orden.IdOrden.ToString(), False)
                Else
                    epNotificador.showWarning("Los registros en el archivo no cumplen con algunas validaciónes")
                    cargarGridErrores(ordSerial.ErroresValidacion)
                End If
            Catch ex As Exception
                If ex.Message = "1" Then
                    epNotificador.showWarning("ya existe una orden de recolección con esa orden de servicio")
                    cargarGridErrores(orden.Referencias.Seriales.ErroresValidacion)
                Else
                    epNotificador.showError("Error al tratar de registrar la orden")
                End If

            End Try

        Else
            epNotificador.showWarning("La recolección  debe tener al menos referencias o accesorios")
        End If
