...
Public Class PoolEliminarVisitasSimpliRoute
...
#Region "Load"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack And Not IsCallback Then
            miEncabezado.setTitle("Pool Eliminar Visitas")
            Try
...
#Region "CallBack"
    Protected Sub cpGeneral_Callback(sender As Object, e As DevExpress.Web.CallbackEventArgsBase)
        ...
        Select Case accion(0)
            Case "1"
                Try
                    CargarServiciosParaEliminarVisita()
                Catch ex As Exception
    ...
    Private Sub gvDatos_CustomCallback(sender As Object, e As DevExpress.Web.ASPxGridViewCustomCallbackEventArgs) Handles gvDatos.CustomCallback
        ...
        Try
            If e.Parameters.Length > 0 Then
                ...
                Dim fieldValues As List(Of Object) = gvDatos.GetSelectedFieldValues("idVisita")
...
#Region "cargue de pool"
    ...
    Private Sub CargarServiciosParaEliminarVisita()
        Dim dtDatos As New DataTable
        Dim objRuta As New RutasSimpleRout
        dtDatos = objRuta.ConsultarServiciosParaEliminarVisita(ObtenerFiltro)
...
#Region "Metodos Privados"
    Private Function ProcesarVisitas(listaVisitas As DataTable) As ResultadoProceso
        ...
        Try
            ...
            With objServicio
                ...
                If rest.Token <> "" Then
                    ...
                    dsDatos = .GenerarPoolEliminarVisitasSimpliRoute(listaVisitas)
                    dtErrores = rest.EliminarVisita(dsDatos)
                    ...
                    Else
                        Dim fieldValues As List(Of Object) = gvDatos.GetSelectedFieldValues("idServicioMensajeria")
                        Dim listaServicio As DataTable = ConvertToDataTable(fieldValues)
                        resultado = .ActualizarEstadoServicioVisitaEliminadaSimpleRute(listaServicio, listaVisitas)
                    ...
                    End If
                Else
                    resultado.EstablecerMensajeYValor(0, "Se generó un error al intentar eliminar Visitas: Pendiete Configuarar URL.")
                    miEncabezado.showError("Se generó un error al intentar eliminar Visitas: Pendiete Configuarar URL.")
                End If
                    ...
                    If numColumnas = totalColumnasxTipo Then ' se valida el numero de columnas  del archivo cargado
                        Try
                            ...
                            If (dterrores IsNot Nothing Or dterrores.Rows.Count < 0) Then
                                ...
                                Dim dtDatos As New DataTable
                                Dim objRuta As New RutasSimpleRout
                                dtDatos = objRuta.ConsultarServiciosParaEliminarVisita(ObtenerFiltroMasivo(dtCargue))
                                Session("dtServiciosVisita") = dtDatos
                                gvDatos.DataSource = dtDatos
                                gvDatos.DataBind()
