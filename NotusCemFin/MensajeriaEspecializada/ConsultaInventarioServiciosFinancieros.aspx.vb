...
Public Class ConsultaInventarioServiciosFinancierosServiciosFinancieros
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                With miEncabezado
                    .setTitle("Consulta de Inventario Servicios Financieros")
...
#Region "Métodos"
    ...
    Private Sub CargarBodega()
        ...
        Try
            dtBodega = HerramientasMensajeria.ConsultarBodega(idCiudad:=CInt(ddlCiudad.Value))
            With ddlBodega
                .DataSource = dtBodega
                .DataBind()
    ...
    Private Sub CargarMaterial(pMaterial As String)
        Try
            ...
            dtMaterial = Subproducto.ObtenerListadoComboMaterialServicioFinancieros(pMaterial)
            With cmbMaterial
    ...
    Private Sub Buscar()
        Try
            ...
            Dim dtDatos As DataTable = objConsulta.ConsultarInventarioProductoFinancieros(False, cantidadRegistros)
    ...
    Private Sub Exportar()
        Try
             If Session("dtInventario") IsNot Nothing AndAlso CType(Session("dtInventario"), DataTable).Rows.Count > 0 Then
                ...
                Dim dtDatos As DataTable = dvDatos.ToTable(False, "bodega", "centro", "almacen", "material", "subproducto", "cantidad", "reserva", "cantidadDisponible")
    ...
    Private Sub ExportarSerializado()
        Try
            ...
            objConsulta.IdEstado = Enumerados.EstadoInventario.LibreUtilizacion
            Dim nunMaxRegis As Int32 = CType(Session("NumeroMaximoregistros"), Int32)
            Dim fec As String = DateTime.Now.ToString("MM/dd/yy HH:mm:ss:fff").Replace(":", "").Replace("/", "")
            If Session("CantidadRegistros") IsNot Nothing AndAlso CType(Session("CantidadRegistros"), Int32) > nunMaxRegis Then
                miEncabezado.showWarning("No es posible exportar la información, supera el número de registros permitidos  " & nunMaxRegis & " por favor seleccione otro filtro ")
            Else
                objConsulta.NombreArchivo = Server.MapPath("~/archivos_planos/" & "ReporteInventarioSateliteSerializado_" & fec & ".xlsx")
                ...
                Dim dvDatos As DataView = objConsulta.ConsultarInventarioProductoFinancieros(True, 0).DefaultView
                Session.Remove("nombreArchivoExportar")
                Session("nombreArchivoExportar") = objConsulta.Resultado.Mensaje.Split("\").GetValue(objConsulta.Resultado.Mensaje.Split("\").Length - 1)
                cbPrincipal.JSProperties("cpOrigen") = "exportar"
                cbPrincipal.JSProperties("cpNombreArchivo") = Session("nombreArchivoExportar")
            End If
        Catch ex As Exception
    ...
    Private Sub cbPrincipal_Callback(sender As Object, e As DevExpress.Web.CallbackEventArgsBase) Handles cbPrincipal.Callback
        ...
        Try
            ...
            Select Case arrayAccion(0)
                ...
                Case "ExportarInformacion"
                    ExportarSerializado()
                    CType(sender, ASPxCallbackPanel).JSProperties("cpOrigen") = "exportar"
                    CType(sender, ASPxCallbackPanel).JSProperties("cpNombreArchivo") = Session("nombreArchivoExportar")
            End Select
        Catch ex As Exception
            miEncabezado.showError("Se generó un error: " & ex.Message)
            resultado.Valor = 1
        End Try
