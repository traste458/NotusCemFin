...
#Region "Eventos"
    ...
    '    Try
    '        Dim control As CheckBox = DirectCast(sender, CheckBox)
    '        Dim row As GridViewRow = CType(control.NamingContainer, GridViewRow)
    '        Dim idControl As String = control.ID
    '        Dim rfv As RequiredFieldValidator = CType(row.FindControl("rfvPlanilla"), RequiredFieldValidator)

    '        Dim controlSeleccion As CheckBox = CType(row.FindControl("chkSeleccion"), CheckBox)
    '        Dim controlLegalizaComcel As CheckBox = CType(row.FindControl("chkLegalizaComcel"), CheckBox)

    '        Select Case idControl
    '            Case "chkSeleccion"
    '                If control.Checked Then
    '                    controlLegalizaComcel.Checked = False
    '                    controlLegalizaComcel.Enabled = False
    '                    rfv.Enabled = True
    '                Else
    '                    controlLegalizaComcel.Enabled = True
    '                    rfv.Enabled = False
    '                End If

    '            Case "chkLegalizaComcel"
    '                If control.Checked Then
    '                    controlSeleccion.Checked = False
    '                    controlSeleccion.Enabled = False
    '                Else
    '                    controlSeleccion.Enabled = True
    '                End If
    '                rfv.Enabled = False
    '        End Select
    '    Catch ex As Exception
    '        epLegalizacionServicio.showError("Imposible realizar selección: " & ex.Message)
    '    End Try
    'End Sub
