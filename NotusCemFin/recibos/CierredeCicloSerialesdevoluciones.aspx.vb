...
Public Class CierredeCicloSerialesdevoluciones
...
#Region "Eventos"
        ...
        Try
            ...
            If Not IsPostBack Or Not IsCallback Then
                With miEncabezado
                    ...
                    .setTitle("Cierre de ciclo seriales de devoluciones")
                End With

                If deFechaCierre.Date = Date.MinValue Then deFechaCierre.Date = New Date(1900, 1, 1)
            End If
    ...
    Private Sub CperroresCallback(sender As Object, e As DevExpress.Web.CallbackEventArgsBase) Handles cperrores.Callback
        Try
            ...
            If Session("dtResultado") Is Nothing Then
                miEncabezado.showSuccess("El Cierre de Ciclo de Devolución se Ejecuto Correctamente")
            Else
                With gvErrores
    ...
    Protected Sub ButtonVerEjemplo_Click(sender As Object, e As EventArgs) Handles ButtonVerEjemplo.Click
        Try
            Dim filename As String = Server.MapPath("~/recibos/images/Cierre de ciclo seriales de devoluciones.xls")
            If filename <> "" Then
    ...
    Private Sub upArchivo_FileUploadComplete(sender As Object, e As DevExpress.Web.FileUploadCompleteEventArgs) Handles upArchivo.FileUploadComplete
        Try
            ...
            If upArchivo.HasFile Then
                ...
                Dim dtCierredeCicloDevolucio As New DataTable()
                ...
                If numColumnas = 1 Then
                    dtCierredeCicloDevolucio = CrearEstructuraDeTabla()
                    ...
                    oWs.ExtractToDataTable(dtCierredeCicloDevolucio, oWs.Rows.Count, ExtractDataOptions.SkipEmptyRows, oWs.Rows(filaInicial), oWs.Columns(columnaInicial))

                    If dtCierredeCicloDevolucio.Rows(0).Item(0).ToString().ToUpper() <> "SERIAL" Then
                        RegError("0", "Verifique que exista una columna llamada Serial  y sea la columna 1 en el archivo, la columna actual se llama: " & dtCierredeCicloDevolucio.Rows(0).Item(0).ToString(), "")
                    End If
                    If Session("dtResultado") Is Nothing Then
                        dtCierredeCicloDevolucio.Rows(0).Delete()
                    Else
                        CType(sender, ASPxUploadControl).JSProperties("cpResultadoProceso") = 0
                        Exit Sub
                    End If

                    dtCierredeCicloDevolucio.Columns.Add(New DataColumn("Fila"))
                    Dim fil As Integer = 1
                    For Each row As DataRow In dtCierredeCicloDevolucio.Rows
                        If regExp.IsMatch(row("SERIAL")) Then
                            row("Fila") = fil
                            fil = fil + 1
                        Else
                            RegError(fil, "El serial tiene caracteres no validos o no cumple con la longitud requerida (IMEI 15, SIM 17)" & dtCierredeCicloDevolucio.Rows(0).Item(0).ToString(), dtCierredeCicloDevolucio.Rows(0).Item(0).ToString())

                        End If
                    Next
                    If Session("dtResultado") Is Nothing Then

                        Dim fe As String = deFechaCierre.Text.ToString()
                        dtCierredeCicloDevolucio.Columns.Add(New DataColumn("idUsuario", GetType(System.Int64), idUsuario))
                        Dim dcfecha As New DataColumn("fechaCierre", GetType(System.DateTime))
                        dcfecha.DefaultValue = deFechaCierre.Date
                        dtCierredeCicloDevolucio.Columns.Add(dcfecha)
                        Dim cargueSeriales As New CierredeCicloDevolucioSeriales()
                        Session("dtResultado") = cargueSeriales.CargarSerialesCierreCicloDevolucion(dtCierredeCicloDevolucio, idUsuario, resultado)
                        If (resultado = 1) Then
                            With gvErrores
                                .DataSource = CType(Session("dtResultado"), DataTable)
                                .DataBind()
                        ...
                        Else
                            With gvErrores
                                ...
                                .DataBind()
                            End With
                            miEncabezado.showError("No se pudo realizar el cierre de ciclo de devolución por favor verificar el log de errores : ")
                            CType(sender, ASPxUploadControl).JSProperties("cpResultadoProceso") = 0
                        End If
